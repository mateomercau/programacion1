...
def agregar_pasajeros():
    ...
    destiny=input("ingrese destino del pasajero: ")
    list_data.append((name,dni,destiny))

#funcion agregar ciudades
def agregar_ciudad():
    city=input("ingrese nombre de la ciudad: ")
    country=input("ingrese el pasi que pertenece la ciuadad: ")
    ...
    for name, dni, destiny in list_data:
        for city, country_destiny in list_ubi:
            if destiny == city and country_destiny == country:
                count += 1
    print(f"La cantidad de pasajeros que viajan al país de {country} es: {count}")
...
purchases=[('Nuria Costa', 5, 1234.5,'Calle 1 – 456'), ('Jorge Russo', 7, 3999, 'Calle 2 – 741')]
